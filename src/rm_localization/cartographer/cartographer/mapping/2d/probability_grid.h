...
class ProbabilityGrid : public Grid2D {
 public:
  ...
  // ★ 本 fork（rm_localization/cartographer）新增：给"清除豁免"用。
  // 与 GetProbability 的区别：本函数允许 cell 已带 kUpdateMarker（本周期已被写过），
  // 会先把 marker 去掉再换算，因此可以在 CastRays 的 miss 循环里安全调用。
  float GetProbabilityWithoutMarker(const Eigen::Array2i& cell_index) const;
