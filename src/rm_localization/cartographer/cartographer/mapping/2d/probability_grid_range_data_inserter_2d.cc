...
// ★ 本 fork 新增：清除豁免。返回 true 表示"这个格子不要写 miss"。
// · min_probability_to_clear <= 0  -> 完全不改变上游行为；
// · 本周期已经被写过的格子（带 kUpdateMarker 位）→ 保持"命中优先"的原语义，直接跳过；
// · 其余格子按当前 P(occupied) 判定：已达到阈值 = 已经确信是障碍 ⇒ 免清。
bool SkipClearing(const ProbabilityGrid& grid,
                  const Eigen::Array2i& cell_index,
                  const float min_probability_to_clear) {
  if (min_probability_to_clear <= 0.f) return false;
  return grid.GetProbabilityWithoutMarker(cell_index) >= min_probability_to_clear;
}
...
void CastRays(const sensor::RangeData& range_data,
  ...
  for (const Eigen::Array2i& end : ends) {
    ...
    for (const Eigen::Array2i& cell_index : ray) {
      if (SkipClearing(*probability_grid, cell_index, min_probability_to_clear)) continue;
...
CreateProbabilityGridRangeDataInserterOptions2D(
  ...
  options.set_insert_free_space(
      parameter_dictionary->HasKey("insert_free_space")
          ...
          : true);
  options.set_min_probability_to_clear(
      parameter_dictionary->HasKey("min_probability_to_clear")
          ? parameter_dictionary->GetDouble("min_probability_to_clear")
          : 0.);
  CHECK_GE(options.min_probability_to_clear(), 0.);
  CHECK_LT(options.min_probability_to_clear(), 1.);
...
void ProbabilityGridRangeDataInserter2D::Insert(
  ...
  CastRays(range_data, hit_table_, miss_table_, options_.insert_free_space(),
           static_cast<float>(options_.min_probability_to_clear()),
           probability_grid);
