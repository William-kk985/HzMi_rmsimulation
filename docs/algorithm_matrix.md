...
**两个"装配级开关"会成倍影响行为，A/B 时一次只动一个**：
- `spin_speed ∈ {5.0, 0.0}`（哨兵小陀螺 / 角速度直通）→ 核心组合 ×2；
- `global_obstacle ∈ {stvl, scan, none}`（全局实时障碍来源）→ 核心组合 ×3；
- 两者都算上：102 × 2 × 3 = **612 种可跑配置**（不含 `*_rviz` 这类纯可视化开关）。
