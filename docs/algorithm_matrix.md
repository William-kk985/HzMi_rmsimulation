...
**结论先说：不建议按 2D/3D 物理重组目录。** 因为维度与技术域是**两个正交的轴** ——
`rm_localization/` 里同时住着 3D 的 LIO 和 2D 的 slam_toolbox；`rm_perception/` 里那个
`pointcloud_to_laserscan` 本身就是"3D→2D 的桥"；硬拆会让 colcon 包路径、launch、install 全部重来，
收益只有"看着整齐"。**正确做法 = 保持"目录=技术域"，用这张表标注维度。**
