...
| 参数（hit / miss / num_range_data） | 曾稳定≥5帧 | 其中末态仍在 | 留存+20帧 | 闪烁中位 | 末态占据格 |
|---|---|---|---|---|---|
...
| 0.85 / 0.30（清除更猛） | — | — | 55.6% | — | 1822（自由格更多但墙掉更快） |
| 0.68 / 0.49（清除几乎关掉） | — | — | 43.0% | — | 自由格只剩 **31**（地图长不出空地） |
| `num_accumulated_range_data`=2 / 3 | — | — | 46.3% / **30.2%** | — | 累积把 0.2~0.3s 的运动糊成一个位姿，得不偿失 |

**结论**

1. 能撬动的只有三处：**①命中票加重（hit_probability ↑）②子图别换那么勤（num_range_data ↑）③关掉清除（自由空间一起没，否决）**。
2. 本次采用 **`hit_probability 0.68 → 0.85` + `num_range_data 30 → 90`**（90 就是上游默认值）：
   预期实心墙格子约**翻倍**、留存+20帧 42%→58%、自由空间不变。
3. ⚠️ **仍有约一半"曾经很实的"墙格子最后会掉出去** —— 这是本传感/世界几何的**结构性上限**
   （低雷达 0.226m + 约 0.4m 挡板 ⇒ 单格证据天生稀疏），不是再调一个参数能消掉的。
   要根治只能动结构性方案：抬高雷达 / 加高挡板 / 换更密且不重复的输入，
   或改 cartographer 源码给"已占据格子"加**清除豁免**（`third_party/` 保持原样，不做）。
4. **别再碰 `missing_data_ray_length`**（空转）；`miss_probability` 0.40 是甜点（0.49 → 自由格长不出来；0.30 → 墙掉更快）。
